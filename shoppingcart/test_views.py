...
class ShoppingcartViewTest(TestCase):
    ...
    def test_checkout_post_success_same_billing_address(self):
        ...
        request = self.factory.get('/')
        request.user = self.user
        order = Order(user=self.user, amount=500, ordered_date=datetime.datetime(2019, 7, 25, tzinfo=pytz.UTC), ordered=False)
        order.save()
        response = self.client.post('/shoppingcart/checkout/', self.checkout_form1)
    ...
    def test_post_success_not_same_billing_save_default_shipping_and_billing(self):
        self.client.post('/accounts/login/', self.credentials, follow=True)
        request = self.factory.get('/')
        request.user = self.user
        order = Order(user=self.user, amount=700, 
                      ordered_date=datetime.datetime(2019, 7, 26, tzinfo=pytz.UTC),
                      ordered=False, use_default_shipping=True, use_default_billing=True)
        order.save()
    ...
    def test_post_success_same_billing_save_default_shipping_and_billing(self):
        self.client.post('/accounts/login/', self.credentials, follow=True)
        request = self.factory.get('/')
        request.user = self.user
        order = Order(user=self.user, amount=700, 
    ...
    def test_post_fail_same_billing_no_shipping_address(self):
        self.client.post('/accounts/login/', self.credentials, follow=True)
        request = self.factory.get('/')
        request.user = self.user
        order = Order(user=self.user, amount=700, 
    ...
    def test_post_fail_not_same_billing_invalid_shipping_address(self):
        self.client.post('/accounts/login/', self.credentials, follow=True)
        request = self.factory.get('/')
        request.user = self.user
        order = Order(user=self.user, amount=700, 
    ...
    def test_post_fail_not_same_billing_invalid_billing_address(self):
        self.client.post('/accounts/login/', self.credentials, follow=True)
        request = self.factory.get('/')
        request.user = self.user
        order = Order(user=self.user, amount=700, 
    ...
    def test_post_fail_invalid_data(self):
        self.client.post('/accounts/login/', self.credentials, follow=True)
        request = self.factory.get('/')
        request.user = self.user
        order = Order(user=self.user, amount=700, 
